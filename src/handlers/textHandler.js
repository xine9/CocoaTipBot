...
const privateChat = ctx => {
  ctx.reply(
    `Hello ${ctx.from.first_name} this is official Cocoa tip bot.\nSee /help for more info.`,
    Markup.keyboard([
      ["/balance", "/help"],
      ["/deposit", "/withdraw"]
    ])
...
const groupChat = async ctx => {
  ...
  const re = /[0-9]+ *cocoa/gi;
  const reComma = /(\d{0,3},)?(\d{3},)?\d{0,3} *cocoa/i;
  const reDot = /\d*\.?\d* *cocoa/gi;
  const reClown = /🍵/g;
  const reCircus = /🍫/g;
...
const tip = async (ctx, amount) => {
  ...
  if (transactionSuccess) {
    msg += `*${fromUser.first_name}* sent ${amount.toLocaleString(
      "en-US"
    )} *COCOA 🍵 to *${toUser.first_name}*`;
  } else {
    console.log("Need more COCOA");
    msg += `*${fromUser.first_name}* you need more *COCOA 🍵*`;
  }
  return msg;
};
