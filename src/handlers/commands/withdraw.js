  ...
  if (args.length == 2) {
    ...
    if (
      ...
      if (amount < withdrawLimit) {
        ...
        //Not enough
        msg += `You don't have enough 🥛COCOA🍫.`;
...
const withdrawTokens = async (session, amount, destSLPaddr) => {
  ...
  session.wallet.cocoaPoints -= amount;
...
const withdrawValidation = async (ctx, session, amount, destSLPaddr) => {
  ...
  if (isSLPAddr) {
    ...
    if (tokenBalance < amount) {
      ...
      let warnMsg =
        "ALERT! Escrow wallet doesn't have enough *COCOA* tokens to make transaction 🔔";
      warnMsg += `\nEscrow balance: ${tokenBalance} COCOA; Amount: ${amount}`;
